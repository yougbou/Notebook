...
//TODO add observer for note counts
                         ...
                         observerMain("Default"), observerImportant("Important")
{
    ...
    //for opening the app for the first time
    if(!ExtractSpecials(collections)){
        mainCollection.SetName("Default");
        importantCollection.SetName("Important");
    }

    //attach observers
    mainCollection.Attach(&observerMain);
    importantCollection.Attach(&observerImportant);
...
bool MainFrame::ExtractSpecials(std::vector<Collection>& collVector) {
    int count = 0;
    for (auto it = collVector.begin(); it != collVector.end();) {
        if (it->GetName() == "Default") {
            mainCollection = *it;
            it = collVector.erase(it);
            count++;
        }
        else if (it->GetName() == "Important") {
            importantCollection = *it;
            it = collVector.erase(it);
            count++;
        }
        else ++it;
...
void MainFrame::PopulateComboBox() {
    for (const auto& coll : collections)
        collectionCombo->Append(coll.GetName());
